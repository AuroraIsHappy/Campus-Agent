...
import { api, type Task, type CalEvent, type SettingsStatus } from "./api";
...
export function MemoryView() {
  ...
  const search = () => {
    ...
    setBusy(true);
    api.recall(query, 10).then((r) => setHits(r.results as never)).catch(() => {}).finally(() => setBusy(false));
  };
  return (
    <div className="mx-auto max-w-3xl p-8">
      <PageHeader title="记忆" subtitle="秘书记住的你的偏好、任务日志和知识。" />
      <Card>
            ...
            {hits.map((h, i) => (
              <li key={i} className="rounded-lg border border-ink-100 p-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="campus-chip">{h.layer}</span>
                </div>
